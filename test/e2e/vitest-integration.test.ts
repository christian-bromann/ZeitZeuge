...
describe("e2e: Vitest integration pipeline", () => {
  ...
  beforeAll(() => {
    ...
    execSync(
      `node --cpu-prof --cpu-prof-dir="${profileDir}" "${RUNNER_PATH}"`,
      { cwd: FIXTURE_DIR, timeout: 30_000, stdio: "pipe" }
    );

    const files = readdirSync(profileDir).filter((f) =>
  ...
  describe("profile parsing", () => {
    ...
    test("computeCorrelationMatrix has significant self time (O(n²) CPU work)", () => {
      const summary = parseCpuProfile(rawProfile, profilePath);
      const fn = summary.hotFunctions.find(
        (f) => f.functionName === "computeCorrelationMatrix"
      );
    ...
    test("buildUserIndex is detected (excessive allocation)", () => {
      const summary = parseCpuProfile(rawProfile, profilePath);
      const fn = summary.hotFunctions.find(
        (f) => f.functionName === "buildUserIndex"
      );
    ...
    test("deduplicateRecords is detected (O(n²) scan)", () => {
      const summary = parseCpuProfile(rawProfile, profilePath);
      const fn = summary.hotFunctions.find(
        (f) => f.functionName === "deduplicateRecords"
      );
  ...
  describe("source classification", () => {
    ...
    test("node internals are classified as 'framework'", () => {
      ...
      const nodeInternals = summary.hotFunctions.filter(
        (f) =>
          f.scriptUrl.startsWith("node:") ||
          f.scriptUrl.includes("node_modules")
      );
  ...
  describe("workspace building", () => {
    test("workspace contains all expected categorized files", async () => {
      ...
      testSources.set(
        ...
        "// mock test source for workspace"
      );
      ...
      try {
        ...
        expect(existsSync(join(rootDir, "timing", "overview.json"))).toBe(
          true
        );
        expect(existsSync(join(rootDir, "profiles", "index.json"))).toBe(
          true
        );
        expect(
          existsSync(join(rootDir, "hot-functions", "global.json"))
        ).toBe(true);
        expect(
          existsSync(join(rootDir, "hot-functions", "application.json"))
        ).toBe(true);
        expect(
          existsSync(join(rootDir, "hot-functions", "dependencies.json"))
        ).toBe(true);
        expect(
          existsSync(join(rootDir, "scripts", "application.json"))
        ).toBe(true);
      } finally {
        workspace.cleanup();
      }
    ...
    test("/hot-functions/application.json contains all application functions", async () => {
      ...
      try {
        ...
        const appHotFunctions: HotFunction[] = JSON.parse(
          readFileSync(
            join(rootDir, "hot-functions", "application.json"),
            "utf-8"
          )
        );
    ...
    test("/scripts/application.json shows application source as top script", async () => {
      ...
      try {
        ...
        const appScripts = JSON.parse(
          readFileSync(
            join(rootDir, "scripts", "application.json"),
            "utf-8"
          )
        );
  ...
  describe("detection quality", () => {
    test("CPU-bound function is among the top 5 hottest", () => {
      const summary = parseCpuProfile(rawProfile, profilePath);
      const top5 = summary.hotFunctions
        .slice(0, 5)
    ...
    test("JSON-heavy function has > 1% self time", () => {
      const summary = parseCpuProfile(rawProfile, profilePath);
      const fn = summary.hotFunctions.find(
        (f) => f.functionName === "normalizePayload"
      );
    ...
    test("application functions account for > 30% of total CPU time", () => {
      ...
      const appFns = summary.hotFunctions.filter((f) =>
        f.scriptUrl.includes(APP_SOURCE_FILENAME)
      );
