...
describe("tracePageLoad", () => {
  ...
  test("injects PerformanceObserver script before navigation", () => {
    const scriptCall = cdp.calls.find(
  ...
  test("skips response bodies larger than 2MB", async () => {
    ...
    const result = await handle.stop();
    const large = result.networkRequests.find((r) =>
      r.url.includes("huge.js")
    );
  ...
  test("starts Chrome Tracing before navigation", () => {
    ...
    expect(tracingCall!.params.traceConfig.includedCategories).toContain(
      "devtools.timeline"
    );
