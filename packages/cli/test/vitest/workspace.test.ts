...
function makeProfileSummary(overrides?: Partial<CpuProfileSummary>): CpuProfileSummary {
  return {
...
function makeCorrelatedProfile(overrides?: Partial<CorrelatedProfile>): CorrelatedProfile {
  return {
...
describe('createVitestWorkspace', () => {
  ...
  test('includes test source files in /tests/', async () => {
    const timing = makeTestTiming();
    const profile = makeCorrelatedProfile();
    const testSources = new Map([['test/example.test.ts', "import { test } from 'vitest';"]]);
  ...
  test('includes source files for hot functions above threshold', async () => {
    ...
    const testSources = new Map([['test/example.test.ts', '// test']]);
    const sourcePaths = new Map([['/project/src/utils.ts', 'export function hotFn() {}']]);
  ...
  test('excludes source files for hot functions below threshold', async () => {
    ...
    const testSources = new Map([['test/example.test.ts', '// test']]);
    const sourcePaths = new Map([['/project/src/minor.ts', 'export function minorFn() {}']]);
