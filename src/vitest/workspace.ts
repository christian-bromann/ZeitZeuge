...
export async function createVitestWorkspace(
  ...
  const gcPercentage =
    totalDuration > 0 ? round((totalGcTime / totalDuration) * 100) : 0;
  ...
  for (const profile of profiles) {
    ...
    files[`/profiles/${safeName}.json`] = JSON.stringify(
      profile.summary,
      null,
      2
    );
  ...
  files["/hot-functions/global.json"] = JSON.stringify(
    mergedHotFunctions,
    null,
    2
  );
  ...
  const appHotFunctions = mergedHotFunctions.filter(
    (fn) => fn.sourceCategory === "application"
  );
  ...
  const depHotFunctions = mergedHotFunctions.filter(
    (fn) => fn.sourceCategory === "dependency"
  );
 ...
 */
export function mergeHotFunctions(
