...
export function zeitzeuge(options: ZeitZeugeVitestOptions = {}) {
  ...
  return {
    ...
    configureVitest(context: any) {
      ...
      const cpuProfArgs = [
        "--cpu-prof",
        `--cpu-prof-dir=${resolvedProfileDir}`,
      ];
      ...
      const existingForksArgv: string[] =
        vitest.config.poolOptions.forks.execArgv ?? [];
      vitest.config.poolOptions.forks.execArgv = [
        ...existingForksArgv,
        ...cpuProfArgs,
      ];
      ...
      if (verbose) {
        console.log(
          `[zeitzeuge] Plugin enabled — CPU profiling to ${resolvedProfileDir}`
        );
        console.log(
          `[zeitzeuge] execArgv: ${JSON.stringify(vitest.config.execArgv)}`
        );
        console.log(
          `[zeitzeuge] pool: ${vitest.config.pool}`
        );
      }
    },
  };
